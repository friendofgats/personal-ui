        ...
        <header className="z-10 w-full">
            <div className="flex max-2xl:flex-col 2xl:flex-row 2xl:m-5 2xl:-mb-1 items-center text-xl text-yellow">
                ...
                <Link className="h-1/2 w-1/2 lg:h-1/3 lg:w-1/3 2xl:h-1/2 2xl:w-1/2 2xl:ml-5" to={"/about"}>
                    <HoverableImage image={togglemenuSelectionImage("about")} alt="Ethan Heyrman" hoverImage={gifs.about} onClick={() => { setMenuSelection("about"); handleMore(false) }} />
                </Link>
                ...
                <div className="flex flex-row items-center lg:h-3/4 lg:w-3/4 2xl:h-full 2xl:w-full max-2xl:ml-2 max-2xl:mr-2">
                    <Link to="/work">
                        <HoverableImage image={togglemenuSelectionImage("work")} alt="What I do for work" hoverImage={gifs.work} onClick={() => { setMenuSelection("work"); handleMore(false) }} />
                    </Link>
                    <Link className="m-5" to="/fun">
                        <HoverableImage image={togglemenuSelectionImage("fun")} alt="What I do for fun" hoverImage={gifs.fun} onClick={() => { setMenuSelection("fun"); handleMore(false) }} />
                    </Link>
                    <div>
                        <MenuCloser clickHandler={handleOffMenuClick}>
                            <HoverableImage className="pb-3" image={togglemenuSelectionImage("more")} alt="More" hoverImage={gifs.more} onClick={() => { setMenuSelection("more"); handleMore(true) }} />
                            <div>
                                {open ? (
                                    <ul className="max-2xl:hidden absolute mt-2 pr-3 pl-3 border-2 border-yellow bg-tan">
                ...
                {open ? (
                    <ul className="flex flex-col items-center lg:hidden pr-2 pl-2 border-2 border-yellow bg-tan">
                        <li className={`focus:text-orange hover:text-brown text-${submenuSelection === "contact" ? "orange" : "yellow"}`}>
                            <Link to="/contact" onClick={() => { setSubmenuSelection("contact"); handleMore(false) }}>Contact me</Link>
                        </li>
                        <li className={`focus:text-orange hover:text-brown text-${submenuSelection === "guts" ? "orange" : "yellow"}`}>
